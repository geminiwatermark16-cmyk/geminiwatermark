...
const { PLAN_AMOUNT, PLAN_CURRENCY, isSandbox, cashfreeRequest, siteUrl, sendError } = require('../lib/cashfree');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const body = {
      ...
      order_meta: { return_url: `${siteUrl(req)}/?cf_order_id=${encodeURIComponent(orderId)}` },
      order_note: 'geminiwatermark.space ₹99 video plan',
      order_tags: { plan: 'video_99' },
    };

    const order = await cashfreeRequest('/orders', {
      method: 'POST',
      headers: { 'x-request-id': crypto.randomUUID(), 'x-idempotency-key': crypto.randomUUID() },
