...
const INTERNATIONAL_PLAN_AMOUNT = 1.09;
...
function isPlanPayment(amount, currency) {
  ...
  return (code === PLAN_CURRENCY && Math.abs(value - PLAN_AMOUNT) < 0.001)
...
function planLabelFromPayment(amount, currency) {
  return String(currency || '').toUpperCase() === INTERNATIONAL_PLAN_CURRENCY
    ? '$1.09 Video — 30 days'
    : '₹99 Video — 30 days';
}
