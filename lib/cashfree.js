...
const SINGLE_PLAN_AMOUNT = 29;
const SINGLE_PLAN_ID = 'video_29_single';
const SINGLE_PLAN_DURATION_DAYS = 1;
const INTERNATIONAL_PLAN_AMOUNT = 1;
const INTERNATIONAL_PLAN_CURRENCY = 'USD';
...
function requestCountry(req) {
  ...
  return /^[A-Z]{2}$/.test(raw) ? raw : 'IN';
}

function planForCountry(country) {
  ...
  return {
    ...
    amount: INTERNATIONAL_PLAN_AMOUNT,
    currency: INTERNATIONAL_PLAN_CURRENCY,
    displayPrice: '$1',
...
function planForRequest(req) {
  return planForCountry(requestCountry(req));
}

function isPlanPayment(amount, currency) {
  const value = Number(amount);
  const code = String(currency || '').toUpperCase();
  return (code === PLAN_CURRENCY && (Math.abs(value - PLAN_AMOUNT) < 0.001 || Math.abs(value - SINGLE_PLAN_AMOUNT) < 0.001))
    || (code === INTERNATIONAL_PLAN_CURRENCY && Math.abs(value - INTERNATIONAL_PLAN_AMOUNT) < 0.001);
}

function planLabelFromPayment(amount, currency) {
  const code = String(currency || '').toUpperCase();
  const value = Number(amount);
  if (code === INTERNATIONAL_PLAN_CURRENCY) return '$1 Video — 30 days';
  if (Math.abs(value - SINGLE_PLAN_AMOUNT) < 0.001) return '₹29 Single Video Pass — 24 hours';
  return '₹99 Video — 30 days';
}
...
function signEntitlement(order, paidAtInput) {
  ...
  const isSingle = Number(order?.order_amount) === SINGLE_PLAN_AMOUNT || String(order?.order_tags?.plan || '').includes('single');
  const durationDays = Number(order?.order_tags?.duration_days) || (isSingle ? SINGLE_PLAN_DURATION_DAYS : PLAN_DURATION_DAYS);
  const planDurationMs = durationDays * 24 * 60 * 60 * 1000;
  const expiresAt = paidAt + planDurationMs;
  const planId = isSingle ? SINGLE_PLAN_ID : PLAN_ID;
  const customer = order?.customer_details || {};
  const dummyPhone = String(order?.order_tags?.customer_phone_dummy || '') === '1';
  const payload = {
    v: 2,
    plan: planId,
    ...
    paidAt,
    expiresAt,
    phone: dummyPhone ? '' : (customer.customer_phone || ''),
...
function inspectEntitlementToken(token) {
  ...
  if (!isPlanPayment(raw.amount, raw.currency)) return null;

  const legacyPlan = raw.v === 1 && raw.plan === 'video_99';
  const currentPlan = raw.v === 2 && (raw.plan === PLAN_ID || raw.plan === SINGLE_PLAN_ID);
  if (!legacyPlan && !currentPlan) return null;

  const durationDays = Number(raw.durationDays) || (Number(raw.amount) === SINGLE_PLAN_AMOUNT ? SINGLE_PLAN_DURATION_DAYS : PLAN_DURATION_DAYS);
  const paidAt = Number(raw.paidAt) || Number(raw.iat);
  if (!Number.isFinite(paidAt) || paidAt <= 0) return null;
  const expiresAt = Number(raw.expiresAt) || (paidAt + durationDays * 24 * 60 * 60 * 1000);
  const active = Date.now() < expiresAt;

  return {
    ...raw,
    plan: raw.plan || (Number(raw.amount) === SINGLE_PLAN_AMOUNT ? SINGLE_PLAN_ID : PLAN_ID),
    paidAt,
    expiresAt,
    active,
    expired: !active,
