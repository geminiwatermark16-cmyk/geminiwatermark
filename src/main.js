...
<main id="top">
  <section class="hero wrap">
    <div class="badge"><i></i> Images free · 1 video free · then ₹99</div>
    <h1>Gemini watermark.<br><span>Gone in seconds.</span></h1>
    <p class="lead">Clean supported visible Gemini watermarks from images and Veo videos directly in your browser. Images stay free. Your first video is free, then video processing unlocks with the ₹99 plan.</p>

    <section id="tool" class="tool">
      <div class="tabs">
        <button id="imageTab" class="active">Image <b>Free</b></button>
        <button id="videoTab">Video <b id="videoBadge">1 Free</b></button>
      </div>
  ...
  <section class="metrics"><div class="wrap"><article><b>100%</b><span>Browser-side media</span></article><article><b>0</b><span>Media files stored</span></article><article><b>1</b><span>Video free</span></article><article><b>₹99</b><span>Video plan</span></article></div></section>
  ...
  <section id="pricing" class="section wrap"><small>PRICING</small><h2>Images free. Start video free.</h2><p class="sectionLead">No image credits. Try one video free on this browser, then unlock video processing for ₹99.</p><div class="pricing"><article><span>IMAGES</span><div class="price"><b>₹0</b><em>forever</em></div><ul><li>Unlimited supported images</li><li>Browser-side processing</li><li>No account required</li></ul><a href="#tool">Remove an image</a></article><article class="featured"><span>VIDEO</span><div class="price"><b>₹99</b><em>video plan</em></div><ul><li>First video free</li><li>Veo visible-mark processing</li><li>Cashfree secure checkout</li></ul><button id="buyPlan">Unlock video for ₹99</button></article></div></section>

  <section id="faq" class="section faq wrap"><small>FAQ</small><h2>Good to know.</h2><details open><summary>Are images really free?</summary><p>Yes. Supported image cleanup has no paid image plan in this version.</p></details><details><summary>How does the free video work?</summary><p>Your first supported video is free on this browser. The current trial is browser-based and can be reset if browser storage is cleared.</p></details><details><summary>Is this affiliated with Google?</summary><p>No. geminiwatermark.space is an independent utility and is not affiliated with, sponsored by, or endorsed by Google.</p></details><details><summary>Does this remove SynthID?</summary><p>No. This tool is for supported visible watermark cleanup only and does not provide invisible provenance or SynthID removal.</p></details></section>
</main>
<footer><div class="wrap"><b>✦ geminiwatermark.space</b><span>Independent utility · © 2026</span></div></footer>

<div id="payModal" class="modal hidden"><div class="modalCard"><button id="closeModal" class="close">×</button><span class="modalKicker">VIDEO PLAN</span><h2>Unlock video</h2><p>Your free video is used. Continue with the ₹99 video plan.</p><div class="payPrice"><b>₹99</b><span>one-time checkout</span></div><label>Mobile number<input id="phone" inputmode="numeric" maxlength="10" placeholder="10-digit mobile number"></label><label>Email <em>optional</em><input id="email" type="email" placeholder="you@example.com"></label><p id="checkoutMsg" class="checkoutMsg"></p><button id="payBtn" class="payBtn">Pay ₹99 with Cashfree</button><small>Secure Cashfree checkout. Plan unlocks only after server verification.</small></div></div>
`;

const $ = id => document.getElementById(id);
const state = { mode: 'image', file: null, result: null, engine: null, paid: false };
const TRIAL_KEY = 'gw_video_free_used_v1';
...
function freeUsed(){ return localStorage.getItem(TRIAL_KEY) === '1'; }
function canVideo(){ return state.paid || !freeUsed(); }
...
function updateVideoUi(){
  ...
  else if(freeUsed()){ $('videoBadge').textContent='₹99'; }
  else $('videoBadge').textContent='1 Free';
  if(state.mode === 'video'){
    $('quotaTitle').textContent = state.paid ? 'Video unlocked' : (freeUsed() ? 'Free video used' : '1 free video');
    $('quotaText').textContent = state.paid ? 'Your ₹99 video plan is active' : (freeUsed() ? 'Upgrade required before another video' : 'Try one supported video free');
    $('quotaPrice').textContent = state.paid ? 'Active' : (freeUsed() ? '₹99' : '₹0');
  }
}
...
function switchMode(mode){
  ...
  if(mode==='image'){
    ...
    $('fileInput').accept='video/mp4,video/webm,video/quicktime'; $('toolTitle').textContent='Drop a video'; $('toolSub').textContent='MP4, WebM or MOV · modern Chrome/Edge recommended'; $('dropStrong').textContent='Drop video here'; $('dropMeta').textContent='First video free · then ₹99 video plan'; updateVideoUi();
  }
}
...
async function processCurrent(){
  ...
  $('processingTitle').textContent = state.mode==='image' ? 'Removing visible mark…' : 'Processing video…';
  try {
    ...
    if(state.mode==='image'){
      ...
      if(!state.paid) localStorage.setItem(TRIAL_KEY,'1');
    }
    state.result=blob; const u=objectUrl(blob);
    if(state.mode==='image') $('afterImg').src=u; else $('afterVideo').src=u;
    $('resultMeta').textContent='Done'; $('downloadBtn').disabled=false; setMessage('Ready to download.','success'); updateVideoUi();
...
$('dropzone').ondrop=e=>{e.preventDefault();$('dropzone').classList.remove('drag'); if(e.dataTransfer.files[0]) select(e.dataTransfer.files[0]);};
